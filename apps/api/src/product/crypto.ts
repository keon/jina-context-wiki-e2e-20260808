...
// access tokens). The key comes from SECRETS_ENCRYPTION_KEY (base64, 32 bytes).
...
function encryptionKey(): Buffer {
  ...
  if (!raw) {
    throw new Error("SECRETS_ENCRYPTION_KEY is required to store secrets");
  }
...
/** Encrypt a string into a self-describing envelope. */
...
export function decryptSecret(value: string): string {
  if (!value.startsWith(ENVELOPE_PREFIX)) {
    throw new Error("stored secret is not an encrypted envelope");
  }
  const key = encryptionKey();
