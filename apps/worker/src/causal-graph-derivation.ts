...
export function issueGraphPrompt(
  ...
  return [
    ...
    "For every ledger entry, emit exactly one concise candidateDisposition. Use issue only when the referenced issue cites that exact commitSha in its evidence. Use duplicate when the commit belongs to an issue already represented but is not itself cited by that issue. Use non_issue for ordinary work despite the lexical signal, and insufficient_evidence only when the commit message cannot support a defensible decision. issue and duplicate dispositions require issueKeys; the other dispositions require an empty issueKeys array.",
