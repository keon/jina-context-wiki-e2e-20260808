...
test("dashboard page renders clickable task detail affordances", () => {
  ...
  const searchableGraph = {
    nodes: [
      ...
      },
      {
        ...
        kind: "File",
        label: "page.ts",
        description: "apps/dashboard/app/page.ts",
      ...
      },
      {
        id: "symbol",
        kind: "Symbol",
        label: "renderOntology",
        description: "function in page.ts",
        path: "apps/dashboard/app/page.ts",
      ...
      },
      {
        id: "issue",
        kind: "Issue",
        label: "Guest denial semantics",
        description: "github issue",
        evidence: ["observation:observation-1"]
      },
      {
        id: "pr",
        kind: "PullRequest",
        label: "#5 Fix guest denial semantics",
        description: "pull request",
        evidence: ["observation:observation-2"]
      },
      {
        id: "commit",
        kind: "Commit",
        label: "d80aa666dd41",
        description: "repo:omxyz/jina:sha:d80aa666dd41a423d2775b8c0c47ba20d53facef",
  ...
  const citedState = {
    citations: [
      { kind: "observation", id: "observation-1", repository: "omxyz/jina" },
      {
        ...
        startLine: 1001,
        endLine: 1001,
        commitSha: "d80aa666dd41a423d2775b8c0c47ba20d53facef"
      }
    ],
    citedClaims: [],
    calls: [
        ...
        items: [
          {
            ...
            data: {
              predicate: "RESOLVES",
              issue: { title: "Guest denial semantics" },
              commitSha: "d80aa666dd41a423d2775b8c0c47ba20d53facef"
            },
  ...
  const citedMatches = new Set(
    contextGraphMatches(citedState, searchableGraph).map((match) => `${match.kind}:${match.id}`)
  );
  ...
  assert.equal(
    citedMatches.has("node:unrelated"),
    false,
    "retrieval-title substrings do not create unsupported graph matches"
  );
  const withoutFiles = filterOntologyGraph(
    searchableGraph,
    new Set(["File", "Symbol"]),
    new Set()
  ) as typeof searchableGraph;
  const filteredMatches = new Set(
    contextGraphMatches(citedState, withoutFiles).map((match) => `${match.kind}:${match.id}`)
  );
  ...
  const graphIdentitySource =
    /function ontologyGraphIdentity\(graph\) \{[\s\S]+?\n\}\n\nfunction resetOntologyViewForGraph/
      .exec(script)?.[0]
      .replace(/\n\nfunction resetOntologyViewForGraph$/, "");
  assert.ok(graphIdentitySource);
  const ontologyGraphIdentity = new Function(`${graphIdentitySource}; return ontologyGraphIdentity;`)() as (graph: {
    id?: string;
    repository: string;
    ref: string;
    commitSha: string;
  ...
  const friendlyLabelSource =
    /function mergePullRequestsForCommit\(node, graph\) \{[\s\S]+?\n\}\n\nfunction friendlyNodeExplanation/
      .exec(script)?.[0]
      .replace(/\n\nfunction friendlyNodeExplanation$/, "");
  ...
  const commit = {
    ...
    kind: "Commit",
    label: "d80aa666dd41",
    description: "repo:omxyz/jina-ontology-e2e:sha:d80aa666dd41a423d2775b8c0c47ba20d53facef"
  };
    ...
    friendlyNodeLabel(
      {
        id: "symbol",
        kind: "Symbol",
        label: "entity:symbol-a",
        description: "repo:omxyz/jina:moniker:src/app.ts:Thing"
      },
      { nodes: [], edges: [] }
    ),
  ...
  const confidenceSource = /function connectedConfidenceSummary\(edges\) \{[\s\S]+?\n\}\n\nfunction ontologyConfidence/
    .exec(script)?.[0]
    .replace(/\n\nfunction ontologyConfidence$/, "");
  ...
  const partitionSource = /function partitionBoardTasks\(tasks\) \{[\s\S]+?\n\}\n\nfunction renderColumns/
    .exec(script)?.[0]
    .replace(/\n\nfunction renderColumns$/, "");
  ...
  const result = partition([
    {
      ...
      metadata: { tenantId: "t", repository: "o/r", ref: "main", requestKey: "old" }
    },
    {
      id: "old-project",
    ...
    },
    {
      ...
      metadata: { tenantId: "t", repository: "o/r", ref: "main", requestKey: "new" }
    },
    {
      id: "new-project",
      type: "ontology_project",
  ...
  const workflowSource = /function buildWorkflowTrees\(definitions\) \{[\s\S]+?\nfunction renderWorkflowTrees/
    .exec(script)?.[0]
    .replace(/\nfunction renderWorkflowTrees$/, "");
  ...
  const buildWorkflowTrees = new Function(`${workflowSource}; return buildWorkflowTrees;`)() as (
    definitions: unknown[]
  ) => {
    name: string;
    roots: WorkflowTreeNode[];
  }[];
  const dependencies = {
    ontology_build: [
      {
    ...
    pr_review: [
      ...
      {
        taskType: "publish",
        relationships: ["blocks", "publishes"],
        workflows: ["pr_review"],
        required: true,
        conditions: []
      }
  ...
  assert.deepEqual(
    reviewRoot?.children[0]?.node.children[0]?.node.children[0]?.node.collapsedDependencies.map(
  ...
  assert.deepEqual(
    ontologyRoot?.children[0]?.node.children[0]?.node.children[0]?.node.collapsedDependencies.map(
      (edge: { from: string }) => edge.from
    ),
    ["ontology_ingest", "ontology_assert"]
  );

  const issueTraceOrderingSource =
    /function isCausationQuestion\(question\) \{[\s\S]+?\n\}\n\nfunction renderIssueTrace/
      .exec(script)?.[0]
      .replace(/\n\nfunction renderIssueTrace$/, "");
  ...
  const issueTraceSections = new Function(`${issueTraceOrderingSource}; return issueTraceSections;`)() as (
    ...
    question: string
  ) => { kind: string; value: { sha?: string; pullRequestNumber?: number } }[];
  const issueTrace = {
    introducedBy: [{ sha: "334234bffedc" }],
    resolutions: [{ pullRequestNumber: 5 }]
  };
  assert.deepEqual(
    issueTraceSections(issueTrace, 'Which PR or commit caused "Administrators cannot delete resources", and why?').map(
...
test("ontology graph settles quickly without continuous camera fitting", () => {
  ...
  const settleDurationSource = /function settleDuration\(nodeCount: number\): number \{[\s\S]+?\n\}/.exec(source)?.[0];
  assert.ok(settleDurationSource);
  const settleDuration = new Function(`${settleDurationSource.replace(/: number/g, "")}; return settleDuration;`)() as (
    nodeCount: number
  ) => number;
  ...
  const topologyLinkDistanceSource = /function topologyLinkDistance\(nodeCount: number\): number \{[\s\S]+?\n\}/.exec(
    source
  )?.[0];
  assert.ok(topologyLinkDistanceSource);
  const topologyLinkDistance = new Function(
    `${topologyLinkDistanceSource.replace(/: number/g, "")}; return topologyLinkDistance;`
  )() as (nodeCount: number) => number;
  ...
  const initialFitPaddingSource = /function initialFitPadding\(nodeCount: number\): number \{[\s\S]+?\n\}/.exec(
    source
  )?.[0];
  assert.ok(initialFitPaddingSource);
  const initialFitPadding = new Function(
    `${initialFitPaddingSource.replace(/: number/g, "")}; return initialFitPadding;`
  )() as (nodeCount: number) => number;
