...
Schema upgrades drop the former `commit_files`, `model_outputs`, and `issue_traces` tables. Their data duplicates canonical storage: historical manifests use exact tree state with churn replay only for pre-migration rows, and model generations live as immutable `model_output` observations. Reviewed `RESOLVED_BY` assertions remain canonical and project alongside deterministic `RESOLVES` facts.
