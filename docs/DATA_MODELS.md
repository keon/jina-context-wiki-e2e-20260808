...
`commits` records the exact path/blob tree observed for every immutable commit, while `commit_changes` stores first-parent churn for change questions and represents a root as additions. `commit_manifest(tenant, repository, sha)` returns the recorded tree directly, with ancestry replay only for pre-migration rows; `ref_manifest` is the rebuildable hot-ref projection. Generic causal, issue, and Feature retrieval traverse the ref's `graph_heads` generation and join canonical assertions, observations, and commit changes for cited detail; there is no second JSON projection. Stable semantic keys include `repo:<repository>:feature:<slug>`, `package:<ecosystem>:<name>`, `service:<source>:<external-id>`, `deployment:<source>:<external-id>`, `incident:<source>:<external-id>`, and `virtual-issue:<repository>:<content-digest>`. Assertions contain an immutable explanation, checked evidence, typed qualifiers, five-state review status, confidence, provenance, generator, validity, supersession, confirmation time, audit linkage, and registry version. `INTRODUCED_BY` requires Issue/VirtualIssue/Incident → Commit/Deployment endpoints and retains its causal explanation as the `reason` qualifier. `assertion_relations` records cited `supports | contradicts` links. Counterfactuals add no table: they synthesize over explained relationships and remove paths from the materialized graph in memory.

Upgraded databases may still contain the former `commit_files`, `model_outputs`, and `issue_traces` tables and historical relationship rows. Current code does not use those caches: historical manifests use exact tree state with churn replay only for pre-migration rows, and model generations live only as `model_output` observations. Current projection accepts both deterministic `RESOLVES` facts and reviewed `RESOLVED_BY` assertions.
