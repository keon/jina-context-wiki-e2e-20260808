...
- Cloud Run: `jina-api`, `jina-dashboard`, `jina-task-worker`, `jina-ontology-worker`
...
Production Ontology uses the `jina-openrouter-api-key` secret with:

```text
ONTOLOGY_CODEX_PROVIDER=openrouter
ONTOLOGY_CODEX_MODEL=deepseek/deepseek-v4-flash
```
...
## Ontology retry and cache behavior
...
After deployment, the workflow checks API health, worker connectivity, dashboard IAP, and the IAP policy. The `jina-acceptance` job receives the internal credential directly from Secret Manager and runs the private fixture repository through the three-stage Ontology workflow.
...
The acceptance poll window is 50 minutes, the Cloud Run task limit is 55 minutes, and production raises the model-command budget from 30 to 40 minutes. `ONTOLOGY_FOCUS_BUNDLE_FILE_LIMIT`, `ONTOLOGY_FOCUS_BUNDLE_MAX_CHARS`, and `ONTOLOGY_FOCUS_BUNDLE_FILE_CHARS` independently bound preloaded evidence.
...
Split services may receive `jina_ontology_intake`, `jina_ontology_code`, `jina_ontology_knowledge`, `jina_ontology_manifest`, `jina_ontology_search`, `jina_ontology_reconciliation`, `jina_ontology_graph`, or `jina_ontology_query`. The modular-monolith login may use aggregate `jina_ontology_writer`; reporting logins use `jina_ontology_reader`. Application logins must not own the schema.
