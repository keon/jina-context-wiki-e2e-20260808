...
complete log inside the Cloud Build step, emits the terminal 400 lines on failure, and
emits only a bounded success record. This keeps the actionable failure or deployment
diagnostics visible in GitHub's size-limited Cloud Build check.
...
| `_JINA_API_DB_POOL_MAX`                                  |                                              `3` | Maximum connections in the shared Context/product pool and shared-identity pool. Context checkout is bounded at ten seconds; JSON Board state owns a dedicated two-connection pool with five-second checkout and thirty-second idle deadlines. |
