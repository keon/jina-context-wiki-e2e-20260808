...
| `context_graph.read`                                                                                                     | API                  | One line per top-level graph query with the authenticated `principalId`, bounded `accessChannel` (`mcp`, `api`, `admin`, or `direct`), tenant, repository, request/trace ID, retrieval templates, and truncation outcome. Query text is never logged.                                                 |
