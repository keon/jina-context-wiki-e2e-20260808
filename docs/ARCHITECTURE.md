...
- `jina-ontology-worker` runs three board-visible chunks: raw-data aggregation, assertion derivation, and projection. Aggregation walks only the previously unseen commit-DAG portion, stores first-parent churn, parses unseen tenant/blob/parser pairs, and deterministically normalizes GitHub work items, CODEOWNERS, manifests, named service definitions, deployments, and explicit incidents. Assertion generation is the sole model stage: it checks out the pinned commit in Daytona, validates citations and typed IDs, retains raw output, and stores supported Feature, VirtualIssue, movement, impact, documentation, ownership, and causal relationships as proposed knowledge. Projection claims repository-scoped canonical events, rebuilds affected refs, reconciles redirects, and creates or reuses the content-addressed graph. Generic causal retrieval roots at Issue, VirtualIssue, Feature, Incident, or Service, while synchronous counterfactuals remove PR/commit/package/deployment/implementation paths from that graph without creating tasks or facts. Stage results distinguish changed work, confirmation, and true no-ops.
