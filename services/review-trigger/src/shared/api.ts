...
export async function postInternal<TResponse = JsonValue>(
  path: string,
  body: unknown,
): Promise<TResponse> {
  ...
  try {
    ...
    logger.info("internal_api_post_completed", internalApiLogPayload({
      path,
      status: response.status,
      durationMs,
      response: parsed,
...
function isSensitiveLogKey(key: string): boolean {
  ...
  return /authorization|auth|cookie|token|secret|api[_-]?key|provider[_-]?key|openai|anthropic|claude|harness/i.test(key);
}
...
function redactSensitiveText(value: string): string {
  return value
    ...
    .replace(/\b(?:token|secret|api[_-]?key|authorization|openai_api_key|anthropic_api_key)\s*[:=]\s*[^,\s}]+/gi, (match) => {
