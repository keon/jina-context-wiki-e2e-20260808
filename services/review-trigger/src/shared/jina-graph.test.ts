...
test("ContextGraph MCP is enabled by default and requests access for the exact installed repository", async () => {
  ...
  const result = await createContextGraphMcpAccess(input, {}, async <TResponse>(nextPath: string, nextBody: unknown) => {
    path = nextPath;
    body = nextBody;
    return {
      available: true,
...
test("ContextGraph MCP is not attached when the API reports no graph", async () => {
  const result = await createContextGraphMcpAccess(input, {}, async <TResponse>() => ({ available: false }) as TResponse);
  assert.equal(result, undefined);
});

test("ContextGraph MCP requires https outside localhost", async () => {
  await assert.rejects(
    () => createContextGraphMcpAccess(input, {}, async <TResponse>() => ({
      available: true,
