...
export async function runReviewContext(input: ReusableCheckoutInput): Promise<CodegraphReviewContextResult> {
  ...
  return withTemporaryCheckout(input, "jina-review-context-", (repoDir) => collectReviewContext(repoDir, input.baseRef));
}
  ...
  try {
    ...
    return {
      markdown: [
        ...
        affectedTestLines.length ? affectedTestLines.map((file) => `- ${file}`).join("\n") : "- None found by codegraph.",
        "",
...
export async function ensurePullRequestCheckout(input: PullRequestCheckoutInput): Promise<void> {
  ...
  await runCommand(
    "git",
    ["fetch", "--no-tags", "origin", `+refs/pull/${input.pullRequestNumber}/head:refs/remotes/origin/pr/${input.pullRequestNumber}`],
    {
      cwd: input.repoDir,
      env: gitEnv,
      timeoutMs: 120_000,
...
function temporaryCheckoutCleanupLabel(prefix: string): string {
  const normalized = prefix.replace(/^jina-/, "").replace(/-$/, "").replaceAll("-", "_");
