...
export function providerFailureCategory(error: string | undefined): ProviderFailureCategory | undefined {
  ...
  const diagnostic = error.slice(0, 12_000);
  if (
    /quota|rate.?limit|too many requests|insufficient[_ ]quota|credit(?:s)? (?:balance|exhausted)|usage limit/i.test(
      diagnostic
    )
  ) {
    return "quota";
  ...
  if (
    /unauthorized|invalid[_ ](?:api[_ ]?)?key|authentication|invalid_grant|token_expired|refresh token|logged out/i.test(
      diagnostic
    )
  ) {
...
export async function runReviewRuntimeStage(
  ...
  try {
    ...
    const repoAccess = await probeRepositoryAccess({ token, repository: payload.repository }).catch(
      failedRepositoryAccess
    );
    await postReviewEvent({
      reviewRunId: payload.review_run_id,
    ...
    if (providerFailure && payload.model_settings?.review_fallback_policy === "managed") {
      ...
      jinaCompleted = true;
      const message =
        `Selected provider failed (${providerFailure}); managed fallback is disabled. ${result.error ?? ""}`.trim();
      await postReviewEvent({
        reviewRunId: payload.review_run_id,
        triggerRunId,
        ...
        payload: {
          ...
          category: providerFailure,
          ...(providerFailure === "quota" ? { quota_reason: providerQuotaReason(result.error) } : {}),
      ...
      await safeUpsertReviewProgressComment({
        ...
        update: {
          ...
          notice: {
            ...
            ...(providerFailure === "quota" ? { quotaReason: providerQuotaReason(result.error) } : {})
          }
        }
      });
    ...
    if (publish.superseded) {
      const outcome = supersededStageResult({
        stage: "runtime",
        startedAt,
        startedAtMs,
        superseded: publish.superseded
      });
      outcome.jinaCompleted = true;
      outcome.publicationStatus = publish.publicationStatus ?? "skipped";
      outcome.publicationReason =
