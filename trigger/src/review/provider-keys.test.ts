...
test("resolveProviderKeys falls back to the OpenRouter key when no harness is connected", async () => {
  stubFetch(() => new Response(JSON.stringify({ codex_harness_auth: null, openrouter_api_key: "or-user-key" }), { status: 200 }));
  const keys = await resolveProviderKeys(42, "run-1");
  assert.deepEqual(keys, { openrouter: "or-user-key", source: "user" });
});
...
test("resolveProviderKeys returns a BYOK native OpenAI key as a 'user' run", async () => {
  ...
  stubFetch(() => new Response(JSON.stringify({ openrouter_api_key: null, openai_api_key: "sk-tenant-openai" }), { status: 200 }));
  const keys = await resolveProviderKeys(42, "run-1");
...
test("resolveProviderKeys carries BOTH company keys when both are present (per-model BYOK routing)", async () => {
  ...
  stubFetch(
    () => new Response(JSON.stringify({ openrouter_api_key: "or-user-key", openai_api_key: "sk-tenant-openai" }), { status: 200 }),
  );
  const keys = await resolveProviderKeys(42, "run-1");
...
test("resolveProviderKeys carries the OpenAI pricing map onto a managed run", async () => {
  const pricing = {
    "openai/gpt-5.4-mini": { input_per_token: "0.0000004", output_per_token: "0.0000016", cached_per_token: "0.0000001" },
  };
  stubFetch(() => new Response(JSON.stringify({ openrouter_api_key: null, openai_model_pricing: pricing }), { status: 200 }));
  const keys = await resolveProviderKeys(42, "run-1");
...
test("resolveProviderKeys omits the pricing map when the API returns null for it", async () => {
  stubFetch(() => new Response(JSON.stringify({ openrouter_api_key: null, openai_model_pricing: null }), { status: 200 }));
  const keys = await resolveProviderKeys(42, "run-1");
  assert.deepEqual(keys, { source: "managed" });
});
