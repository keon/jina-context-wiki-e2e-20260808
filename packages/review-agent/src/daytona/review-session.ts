...
import type { CodegraphReviewContextResult, PullRequestReviewInput } from "../review/codex-review.js";
...
export function ensureModelCredentials(input: {
  ...
  if (!input.openrouterKey && !input.openaiKey) {
    throw new Error("OPENROUTER_API_KEY is required for Codex inside Daytona. Set OPENROUTER_API_KEY on the worker.");
  }
}
  ...
  try {
    ...
    if (!daytonaSkipInstall()) {
      ...
      if (install.exitCode !== 0) {
        throw new Error(
          `Daytona worker dependency install failed: ${redact(daytonaProcessFailureMessage(install.result), secrets)}`
        );
      }
    }
  } catch (error) {
...
async function writeWorkerFiles(sandbox: Sandbox, files: Array<{ path: string; source: Buffer }>): Promise<void> {
...
async function writeWorkerFile(sandbox: Sandbox, remotePath: string, source: Buffer): Promise<void> {
  ...
  for (let offset = 0; offset < encoded.length; offset += chunkSize) {
    ...
    if (append.exitCode !== 0) {
      throw new Error(
        `Daytona worker file write failed for ${remotePath}: ${daytonaProcessFailureMessage(append.result)}`
      );
    }
  }
}
...
function workerSourceCandidates(name: DaytonaWorkerSourceName): Array<string | URL> {
  ...
  return [
    ...
    new URL(`../../${file.sourcePath.replace(/^\.\//, "")}`, import.meta.url),
...
export function resolveDaytonaWorkerToolBins(env: NodeJS.ProcessEnv = process.env): {
  codexBin: string;
  codegraphBin: string;
} {
