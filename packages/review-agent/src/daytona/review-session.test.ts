...
test("resolveOpenrouterKeyForRun: precedence harness > tenant openrouter > byok-placeholder > managed", () => {
  ...
  assert.equal(resolveOpenrouterKeyForRun({ harnessMode: false, managedOpenrouterKey: "sk-managed" }), "sk-managed");
});
...
test("resolveDaytonaSandboxImage honors an explicit custom image", () => {
  assert.equal(
    resolveDaytonaSandboxImage({ DAYTONA_SANDBOX_IMAGE: "ghcr.io/acme/review-worker:latest" }),
    "ghcr.io/acme/review-worker:latest"
  );
});
...
test("daytonaWorkerEnv exposes only the short-lived ContextGraph token and its MCP config references the env name", () => {
  ...
  assert.match(config, /enabled_tools = \["search_context", "list_context", "read_context", "diff_context"\]/);
...
test("collectCodexHarnessTokenSecrets extracts nested token values for redaction", () => {
  ...
  assert.deepEqual(
    collectCodexHarnessTokenSecrets(authJson).sort(),
    ["access-token-value-123456", "id-token-value-123456", "refresh-token-value-123456"].sort()
  );
});
...
test("daytonaWorkerEnv omits the native OpenAI envs in harness mode even if a key is passed", () => {
  const env = daytonaWorkerEnv({
    ...
    openaiKey: "sk-openai-managed",
    openaiModelPricing: {
      "openai/gpt-5.4-mini": { input_per_token: "1", output_per_token: "1", cached_per_token: "1" }
    },
    harnessMode: true,
    reviewRunId: "run-123"
  });
