...
export async function startOpenRouterProxy(options: StartOpenRouterProxyOptions = {}): Promise<OpenRouterProxyHandle> {
  ...
  const openaiModelPricing =
    options.openaiModelPricing ?? parseOpenaiModelPricingEnv(process.env.JINA_OPENAI_MODEL_PRICING);
...
async function handleRequest(req: IncomingMessage, res: ServerResponse, ctx: HandleContext): Promise<void> {
  ...
  return new Promise<void>((resolve) => {
    ...
    const issue = (issueTarget: URL, issueHeaders: http.OutgoingHttpHeaders, issueBody: Buffer): void => {
      ...
      const upstreamReq = transport.request(issueTarget, { method: req.method, headers: issueHeaders }, (res2) => {
        upstreamRes = res2;
...
async function backfillMissingUsage(records: InternalRecord[], ctx: { upstream: URL; apiKey?: string }): Promise<void> {
