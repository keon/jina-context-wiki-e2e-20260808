...
export function stageResult(
  input: Omit<ReviewStageResult, "completedAt" | "durationMs"> & {
    startedAtMs: number;
  }
): ReviewStageResult {
  return {
...
export async function newerManualReviewSuperseded(
  input: ManualReviewSupersession & {
    headSha: string;
  }
): Promise<ReviewSuperseded | undefined> {
  const page = await postInternal<{ runs: ManualReviewRun[] }>("/internal/reviews/manual-runs", {
...
export function isSupersededStageResult(result: ReviewStageResult): boolean {
  return result.status === "skipped" && (Boolean(result.superseded) || isSupersededReason(result.skippedReason));
}
