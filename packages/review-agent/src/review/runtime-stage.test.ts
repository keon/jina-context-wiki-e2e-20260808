...
const usageRecords: UsageRecord[] = [{ operation: "planner", request_seq: 1, cost: "0.01", raw_usage: {} }];
...
test("codexHarnessReconnectRequired recognizes permanent native Codex auth failures", () => {
  assert.equal(codexHarnessReconnectRequired("harness", "auth error code: token_expired; please sign in again"), true);
  assert.equal(
    codexHarnessReconnectRequired(
      "harness",
...
test("codexHarnessReconnectRequired ignores gateway and transient failures", () => {
  ...
  assert.equal(
    codexHarnessReconnectRequired(
      "harness",
      "codex_login::auth::manager: Failed to refresh token: network unavailable"
    ),
    false
  );
