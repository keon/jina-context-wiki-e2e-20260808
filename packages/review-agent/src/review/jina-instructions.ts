...
export const JINA_INSTRUCTION_STEPS = ["planner", "replanner", "investigation", "review"] as const;
...
const KNOWN_INSTRUCTION_PATHS = new Set([GLOBAL_INSTRUCTION_PATH, ...Object.values(STEP_INSTRUCTION_PATHS)]);
...
export async function loadJinaRuntimeConfigAtRef(
  ...
  const runtimeConfig = await loadJinaRuntimeConfig(repoDir, ref, blobSizes.get(JINA_CONFIG_PATH), warnings);
...
async function loadJinaRuntimeConfig(
  ...
  if (size > FILE_READ_MAX_BYTES) {
    warnings.push(
      `${JINA_CONFIG_PATH} is ${size} bytes, over the ${FILE_READ_MAX_BYTES}-byte read cap; using defaults.`
    );
 ...
 */
export function appendJinaInstructionsToPrompt(defaultPrompt: string, instructionsMarkdown?: string): string {
...
async function readInstructionFile(
  ...
  if (size > FILE_READ_MAX_BYTES) {
    warnings.push(`${filePath} is ${size} bytes, over the ${FILE_READ_MAX_BYTES}-byte read cap, and was skipped.`);
    return undefined;
  }
  ...
  try {
    ...
    const content = truncateInstructionBody(cleanInstructionBody(result.stdout), filePath, warnings);
...
async function listJinaBlobSizes(repoDir: string, ref: string, warnings: string[]): Promise<Map<string, number>> {
  try {
