...
test("supersededDetails detects head changes", () => {
  assert.deepEqual(supersededDetails({ state: "open", merged: false, head: { sha: "def456789abc" } }, "abc123789abc"), {
...
test("supersededDetails detects merged and closed pull requests", () => {
  assert.deepEqual(supersededDetails({ state: "closed", merged: true, head: { sha: "abc123" } }, "abc123"), {
  ...
  assert.deepEqual(supersededDetails({ state: "closed", merged: false, head: { sha: "abc123" } }, "abc123"), {
