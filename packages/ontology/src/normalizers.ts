...
export interface ServiceDefinitionObservation {
  ...
  readonly dependsOnServices?: readonly { readonly source: string; readonly externalId: string; readonly name: string }[];
...
export interface MoveCandidateObservation {
  ...
  readonly commitSha: string;
  readonly candidates: readonly { readonly oldPath: string; readonly newPath: string; readonly similarity: number; readonly matchingSignatureHashes: readonly string[] }[];
  readonly recordedAt: string;
}

export type RepositorySourceObservation = GitHubSourceObservation | PackageManifestObservation |
  ServiceDefinitionObservation | DeploymentSourceObservation | IncidentSourceObservation | MoveCandidateObservation;
...
export function sourceObservationExternalId(observation: RepositorySourceObservation): string {
  if (observation.kind === "codeowners") return `${observation.repository}:codeowners:${observation.commitSha}:${observation.path}`;
  ...
  if (observation.kind === "move_candidate") return `${observation.repository}:${observation.kind}:${observation.commitSha}`;
...
export function parsePackageManifest(input: Omit<PackageManifestObservation, "kind" | "ecosystem" | "dependencies"> & {
  readonly source: string;
...
export function parseServiceDefinitions(input: Omit<ServiceDefinitionObservation, "kind" | "source" | "externalId" | "name"> & {
  readonly content: string;
}): readonly ServiceDefinitionObservation[] {
  const path = input.path.toLowerCase();
  const values: { source: string; externalId: string; name: string; dependsOnServices?: ServiceDefinitionObservation["dependsOnServices"] }[] = [];
  const dockerfile = /(?:^|\/)(?:dockerfile)(?:\.([A-Za-z0-9_.-]+))?$/i.exec(input.path);
  if (dockerfile) {
    const directory = input.path.includes("/") ? input.path.slice(0, input.path.lastIndexOf("/")).split("/").at(-1) : undefined;
    const name = dockerfile[1] ?? (directory && !/^(?:docker|deploy(?:ment)?s?)$/i.test(directory) ? directory : undefined);
  ...
  if (/(?:^|\/)(?:docker-)?compose(?:\.[^.]+)?\.ya?ml$/.test(path)) {
    ...
    for (const [name, dependencies] of services) values.push({
      source: "compose", externalId: `${input.repository}:${name}`, name,
      dependsOnServices: [...dependencies].map((dependency) => ({
        source: "compose", externalId: `${input.repository}:${dependency}`, name: dependency
      }))
    });
  }
  if (/\.ya?ml$/.test(path)) {
    const kind = /^kind:\s*(Service|Deployment)\s*$/mi.exec(input.content)?.[1];
    const name = /^\s*name:\s*([A-Za-z0-9_.-]+)\s*$/mi.exec(input.content)?.[1];
    const cloudRunName = /^\s*metadata:\s*$[\s\S]{0,500}?^\s*name:\s*([A-Za-z0-9_.-]+)\s*$/mi.exec(input.content)?.[1];
  ...
  if (/^\.github\/workflows\/.*\.ya?ml$/.test(path)) {
    ...
    for (const match of input.content.matchAll(/uses:\s*google-github-actions\/deploy-cloudrun@[^\n]+[\s\S]{0,800}?^\s*service:\s*["']?([A-Za-z0-9_.-]+)["']?\s*$/gmi)) {
      if (match[1]) serviceNames.add(match[1]);
    }
  ...
  return dedupe(values, (value) => `${value.source}:${value.externalId}`).map((value) => ({
    tenantId: input.tenantId, repository: input.repository, kind: "service_definition",
    commitSha: input.commitSha, path: input.path, recordedAt: input.recordedAt,
    source: value.source, externalId: value.externalId, name: value.name,
...
export function parseIncidentDocument(input: {
  ...
  if (!/(?:^|\/)(?:incidents?|postmortems?)(?:\/|[-_.])/i.test(input.path) || !/\.md(?:own)?$/i.test(input.path)) return undefined;
  const externalId = /^(?:incident[-_ ]?id|id):\s*["']?([A-Za-z0-9_.-]+)["']?\s*$/mi.exec(input.content)?.[1] ??
    /\b(INC(?:IDENT)?[-_ ]?\d{2,})\b/i.exec(input.content)?.[1];
  if (!externalId) return undefined;
  const title = /^#\s+(.+)$/m.exec(input.content)?.[1]?.trim() ?? externalId;
  const serviceName = /^(?:service|impacted[-_ ]?service):\s*["']?([A-Za-z0-9_.-]+)["']?\s*$/mi.exec(input.content)?.[1];
  const serviceSource = /^service[-_ ]?source:\s*["']?([A-Za-z0-9_.-]+)["']?\s*$/mi.exec(input.content)?.[1] ?? "postmortem";
  const serviceExternalId = /^service[-_ ]?external[-_ ]?id:\s*["']?([^"'\n]+)["']?\s*$/mi.exec(input.content)?.[1];
  const issueNumberText = /^(?:github[-_ ]?)?issue:\s*#?(\d+)\s*$/mi.exec(input.content)?.[1];
  return {
    tenantId: input.tenantId, repository: input.repository, kind: "incident", source: "postmortem",
    externalId: `${input.repository}:${externalId.toLowerCase().replaceAll(" ", "-")}`, title,
    ...(serviceName ? { impactedService: {
      source: serviceSource, externalId: serviceExternalId ?? `${input.repository}:${serviceName}`, name: serviceName
    } } : {}),
    ...(issueNumberText ? { issueNumber: Number.parseInt(issueNumberText, 10) } : {}),
    occurredAt: input.recordedAt, recordedAt: input.recordedAt
  };
}
...
function normalizeServiceDefinition(observation: ServiceDefinitionObservation): NormalizedGitHubObservation {
  const service: SourceEntityIntent = {
    kind: "Service", key: `service:${observation.source}:${observation.externalId}`, displayName: observation.name
  };
  const dependencies = (observation.dependsOnServices ?? []).map((dependency): SourceEntityIntent => ({
    kind: "Service", key: `service:${dependency.source}:${dependency.externalId}`, displayName: dependency.name
  }));
  return {
...
function normalizeDeployment(observation: DeploymentSourceObservation): NormalizedGitHubObservation {
  const deployment: SourceEntityIntent = {
    kind: "Deployment", key: `deployment:${observation.source}:${observation.externalId}`,
    displayName: `${observation.environment} deployment ${observation.externalId}`
  };
  const commit: SourceEntityIntent = {
    kind: "Commit", key: `repo:${observation.repository}:sha:${observation.commitSha}`, displayName: observation.commitSha.slice(0, 12)
  };
...
function packageDependencies(path: string, source: string): {
  readonly ecosystem: string;
  readonly dependencies: readonly { readonly name: string; readonly version?: string }[];
} | undefined {
  ...
  if (name === "package.json") {
    ecosystem = "npm";
    let value: unknown;
    try { value = JSON.parse(source); } catch { return { ecosystem, dependencies: [] }; }
    ...
    for (const section of sections) for (const match of section.matchAll(/^([A-Za-z0-9_.-]+)\s*=\s*["']([^"']+)["']/gm)) {
      if (match[1] && match[1].toLowerCase() !== "python") dependencies.push({ name: match[1], ...(match[2] ? { version: match[2] } : {}) });
    }
    ...
    const dependencyArray = dependencyEnd >= dependencyStart ? projectSection.slice(dependencyStart, dependencyEnd) : "";
    ...
    for (const match of source.matchAll(/^\s*([A-Za-z0-9_.~/-]+)\s+(v[^\s]+)\s*(?:\/\/.*)?$/gm)) {
      if (match[1] && !match[1].startsWith("module")) dependencies.push({ name: match[1], ...(match[2] ? { version: match[2] } : {}) });
    }
  } else if (name === "cargo.toml") {
    ecosystem = "cargo";
    const section = /\[(?:dev-|build-)?dependencies\]([\s\S]*?)(?=\n\[|$)/g;
    for (const block of source.matchAll(section)) for (const match of (block[1] ?? "").matchAll(/^([A-Za-z0-9_-]+)\s*=\s*(?:["']([^"']+)["']|\{[^\n]*version\s*=\s*["']([^"']+))/gm)) {
      if (match[1]) dependencies.push({ name: match[1], ...((match[2] ?? match[3]) ? { version: match[2] ?? match[3] } : {}) });
    }
  } else if (name === "gemfile.lock") {
    ecosystem = "rubygems";
    for (const match of source.matchAll(/^ {4}([A-Za-z0-9_.-]+) \(([^)]+)\)$/gm)) if (match[1]) dependencies.push({ name: match[1], ...(match[2] ? { version: match[2] } : {}) });
  } else if (name === "pom.xml") {
    ecosystem = "maven";
    for (const match of source.matchAll(/<dependency>[\s\S]*?<groupId>([^<]+)<\/groupId>[\s\S]*?<artifactId>([^<]+)<\/artifactId>[\s\S]*?(?:<version>([^<]+)<\/version>)?[\s\S]*?<\/dependency>/g)) {
      if (match[1] && match[2]) dependencies.push({ name: `${match[1]}:${match[2]}`, ...(match[3] ? { version: match[3] } : {}) });
    }
  } else if (/^(?:build\.gradle(?:\.kts)?|settings\.gradle(?:\.kts)?)$/.test(name)) {
    ecosystem = "maven";
    for (const match of source.matchAll(/(?:implementation|api|compileOnly|runtimeOnly)\s*\(?["']([^:"']+):([^:"']+)(?::([^"']+))?["']/g)) {
      if (match[1] && match[2]) dependencies.push({ name: `${match[1]}:${match[2]}`, ...(match[3] ? { version: match[3] } : {}) });
    }
