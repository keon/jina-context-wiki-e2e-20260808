...
export function buildCausalTrace(graph: ContextGraph, root: ContextGraphNode): CausalTraceProjection {
  ...
  const causes = (outgoing.get(root.id) ?? [])
    .filter((edge) => edge.predicate === "INTRODUCED_BY")
    .flatMap((edge) => {
      const cause = nodes.get(edge.target);
      if (!cause) return [];
      const containing = (incoming.get(cause.id) ?? []).filter(
        (candidate) => candidate.predicate === "INCLUDES" || candidate.predicate === "MERGED_AS"
      );
  ...
  const resolutions = [
    ...(outgoing.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "RESOLVED_BY")
      .flatMap((edge) => direct(edge, "resolution") ?? []),
    ...(incoming.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "RESOLVES")
      .flatMap((edge) => {
  ...
  const implementations = [
    ...(incoming.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "IMPLEMENTS")
      .flatMap((edge) => {
    ...
    ...(outgoing.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "IMPLEMENTS")
      .flatMap((edge) => direct(edge, "implementation") ?? [])
  ];

  const affectedEntities = [
    ...(incoming.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "LIKELY_AFFECTS")
      .flatMap((edge) => {
        const change = nodes.get(edge.source);
        return change ? [path("impact", [root, change], [edge], graph)] : [];
      }),
    ...(outgoing.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "LIKELY_AFFECTS" || edge.predicate === "INCIDENT_IMPACTS")
      .flatMap((edge) => direct(edge, "impact") ?? []),
    ...(incoming.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "INCIDENT_IMPACTS")
      .flatMap((edge) => {
    ...
    for (const dependencyEdge of (outgoing.get(file.id) ?? []).filter((edge) => edge.predicate === "IMPORTS")) {
      ...
      if (dependency?.kind === "Package")
        dependencies.push(
          path(
            "dependency",
            [root, implementationNode, file, dependency],
            [
  ...
  const deployments = [
    ...(incoming.get(root.id) ?? [])
      .filter((edge) => edge.predicate === "TARGETS")
      .flatMap((edge) => {
        const deployment = nodes.get(edge.source);
        return deployment ? [path("deployment", [root, deployment], [edge], graph)] : [];
      }),
    ...causes
      .filter((candidate) => candidate.nodes.some((node) => node.kind === "Deployment"))
      .map((candidate) => ({ ...candidate, kind: "deployment" as const }))
  ];
      ...
      for (const target of (incoming.get(service.id) ?? []).filter((edge) => edge.predicate === "TARGETS")) {
        ...
        if (deployment)
          deployments.push(
            path(
              "deployment",
              [root, service, deployment],
              [graph.edges.find((edge) => edge.id === impact.edgeIds[0])!, target].filter(Boolean),
              graph
            )
          );
      }
  ...
  const documentation = (outgoing.get(root.id) ?? [])
    .filter((edge) => edge.predicate === "DOCUMENTED_BY")
    .flatMap((edge) => direct(edge, "documentation") ?? []);
  const ownership = (outgoing.get(root.id) ?? [])
    .filter((edge) => edge.predicate === "OWNED_BY")
    ...
    for (const move of (outgoing.get(implementationNode.id) ?? []).filter((edge) => edge.predicate === "MOVED_FROM")) {
      ...
      if (previous)
        movedFrom.push(
          path(
            "move",
            [root, implementationNode, previous],
            [graph.edges.find((edge) => edge.id === implementation.edgeIds[0])!, move].filter(Boolean),
            graph
          )
        );
    }
    for (const structural of [
      ...(outgoing.get(implementationNode.id) ?? []),
      ...(incoming.get(implementationNode.id) ?? [])
      ...
      if (other)
        structuralPaths.push(
          path(
            "structure",
            [root, implementationNode, other],
            [graph.edges.find((edge) => edge.id === implementation.edgeIds[0])!, structural].filter(Boolean),
            graph
          )
        );
    }
  ...
  return {
    root: causalNode(root),
    causes: groups[0]!,
    resolutions: groups[1]!,
    implementations: groups[2]!,
    affectedEntities: groups[3]!,
    dependencies: groups[4]!,
    deployments: groups[5]!,
    documentation: groups[6]!,
    ownership: groups[7]!,
    movedFrom: groups[8]!,
    structuralPaths: groups[9]!,
    citations
  };
}
...
export function evaluateCounterfactual(trace: CausalTraceProjection, question: string): CounterfactualEvaluation {
  ...
  if (interventionMatches.length !== 1) {
    return {
      ...
      outcome: trace.root,
      removedPaths: [],
      remainingPaths: allPaths,
      citedClaims: [],
      unresolvedAmbiguities: interventionMatches.length > 1 ? ["Select one intervention entity."] : [],
      coverageGaps:
        interventionMatches.length === 0 ? ["The intervention is not present in this materialized causal trace."] : []
    };
  }
  ...
  if (removedPaths.length > 0 && removedCitations.length > 0)
    citedClaims.push({
...
function counterfactualPaths(trace: CausalTraceProjection, intervention: CausalTraceNode): readonly CausalTracePath[] {
  if (intervention.kind === "PullRequest" || intervention.kind === "Commit" || intervention.kind === "Deployment") {
    ...
    const matchingGroups = groups.filter((group) =>
      group.some((candidate) => candidate.nodes.some((node) => node.id === intervention.id))
    );
...
function counterfactualAnswer(
  ...
  if (intervention.kind === "Package") {
    if (removedPaths.length === 0)
      return `${intervention.label} is present, but no reviewed implementation dependency path uses it.`;
  ...
  if (intervention.kind === "File" || intervention.kind === "Symbol") {
    if (removedPaths.length === 0)
      return `${intervention.label} is present, but no reviewed implementation path depends on it.`;
  ...
  if (removedPaths.length === 0)
    return `${intervention.label} is present in the trace, but no reviewed path to ${trace.root.label} depends on it.`;
  if (remainingPaths.length === 0)
    return `Removing ${intervention.label} eliminates every currently known reviewed path to ${trace.root.label}. This does not prove the outcome could never occur through an unknown path.`;
...
function selectRoots(graph: ContextGraph, request: RetrievalRequest): ContextGraphNode[] {
  ...
  const packageName = /\bpackage\s+["'`]?([^"'`,?]+?)["'`]?(?:\s+(?:were|was|is|had|excluded|removed)|,|\?|$)/i
    .exec(query)?.[1]
    ?.trim()
    .toLowerCase();
  return candidates
    .map((node) => {
      ...
      if (
        ...
        traceEdges.some((edge) => {
          ...
          return (
            other?.kind === "PullRequest" &&
            new RegExp(`(?:#|:)${pullRequestNumber}(?:\\b|$)`).test(`${other.label} ${other.description} ${other.id}`)
          );
        })
      )
        score += 120;
        ...
        traceEdges.some((edge) => {
          ...
          return graph.edges.some((container) => {
            ...
            if (
              ...
              new RegExp(`(?:#|:)${pullRequestNumber}(?:\\b|$)`).test(
                `${pullRequest.label} ${pullRequest.description} ${pullRequest.id}`
              )
            )
              return true;
      ...
      if (
          ...
          return Boolean(
            ...
            graph.edges.some((edge) => {
              ...
              return (
                ...
                `${dependency.label} ${dependency.description}`.toLowerCase().includes(packageName)
              );
            })
          );
        })
        ...
        score += 120;
      if (
        /\b(?:renamed?|moved?|previously)\b/.test(query) &&
        implementationEdges.some((implementation) =>
...
function selectInterventionNodes(nodes: readonly CausalTraceNode[], question: string): CausalTraceNode[] {
  ...
  const packageName = /\bpackage\s+["'`]?([^"'`,?]+?)["'`]?(?:\s+(?:were|was|is|had|excluded|removed)|,|\?|$)/i
    ...
    ?.trim()
    .toLowerCase();
  const deploymentName = /\bdeployment\s+["'`#]?([^"'`,?]+?)["'`]?(?:\s+(?:were|was|is|had|excluded|removed)|,|\?|$)/i
    .exec(question)?.[1]
    ?.trim()
    .toLowerCase();
  const implementationName =
    /\b(?:symbol|file|implementation(?:\s+path)?)\s+["'`]?([^"'`,?]+?)["'`]?(?:\s+(?:were|was|is|had|disabled|excluded|removed)|,|\?|$)/i
      .exec(question)?.[1]
      ?.trim()
      .toLowerCase();
  ...
  return edge.evidence.flatMap((value): RetrievalCitation[] => {
    if (value.startsWith("assertion:"))
      return [
        {
          kind: "assertion",
          id: value.slice("assertion:".length),
          repository: graph.repository,
          commitSha: graph.commitSha
        }
      ];
    if (value.startsWith("observation:"))
      return [{ kind: "observation", id: value.slice("observation:".length), repository: graph.repository }];
    const match = /^(.*):(\d+)(?:-(\d+))?$/.exec(value);
    if (!match?.[1] || !match[2]) return [];
    return [
      {
        kind: "code",
        id: `${graph.commitSha}:${value}`,
        repository: graph.repository,
        commitSha: graph.commitSha,
        path: match[1],
...
function dedupePaths(paths: readonly CausalTracePath[]): CausalTracePath[] {
  ...
  for (const candidate of paths)
    selected.set(
      `${candidate.kind}:${candidate.nodes.map((node) => node.id).join(">")}:${candidate.edgeIds.join(">")}`,
      candidate
    );
